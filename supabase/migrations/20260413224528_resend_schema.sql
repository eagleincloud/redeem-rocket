...
-- FIXED: Removed foreign key constraints that assume specific table schemas
-- These will be added after verifying the businesses and outreach_campaigns tables exist
...
-- NOTE: Foreign keys will reference the actual existing tables

CREATE TABLE IF NOT EXISTS outreach_email_tracking (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id TEXT, -- References outreach_campaigns(id) - flexible type
  recipient_email TEXT NOT NULL,
  business_id TEXT NOT NULL, -- References businesses(id) - matches existing schema
...
-- RLS is optional and can be enabled per table after verifying schema compatibility

-- Enable RLS on tables with clear business_id isolation
...
-- Note: email_tracking and email_unsubscribe_tokens RLS policies
-- require schema knowledge of external tables (outreach_campaigns, businesses)
-- Enable and configure RLS policies after verifying external table schemas

-- RLS Policies for tables with TEXT business_id
